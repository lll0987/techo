...
import { z } from 'zod';
import { topicSchema, topicSortSchema, useTopicService } from '../../database/topic';

const router = express.Router();
const { list, create, batchCreate, update, remove } = useTopicService();
...
router.post('/list', (req, res) => {
    try {
        const { filter, sort } = z
            .object({
                filter: topicSchema.partial().optional(),
...
router.post('/batchCreate', (req, res) => {
    try {
        const data = topicSchema.array().parse(req.body);
